...
export function Footer() {
  return (
    <footer id="contact" className="w-full scroll-mt-24">
      <Separator />
      <div className="flex flex-col items-center justify-between gap-6 pt-6 text-sm text-muted-foreground sm:flex-row">
        ...
        <div className="flex items-center gap-2">
          {SOCIALS.map(({ href, label, icon: Icon }) => (
            <a
              ...
              className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-800 text-zinc-400 transition-colors hover:border-cyan-800/50 hover:text-cyan-400"
            >
      ...
      <p className="pt-6 text-center text-xs text-zinc-600">
        © {2026} Meheret Tesfaye Batu · Built with Next.js &amp; Tailwind CSS
      </p>
    </footer>
  );
}
